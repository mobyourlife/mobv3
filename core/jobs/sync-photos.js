...
var startSyncing = function (records, callback) {
    ...
    for (i = 0; i < records.length; i += 1) {
        ...
        if (!cur.latest_sync || cur.time > cur.latest_sync) {
            syncAlbumPhotos(cur);
        }
    }
};
...
var job = {
    ...
    checkConditions: function (callback) {
        ...
        Fanpage.find({ 'billing.expiration': { $gt: new Date() }, 'jobs.new_site_created': { $exists: true, $ne: null } }, function (err, pages) {
            ...
            Album.find({ 'ref': { $in: pages_list }, 'count': { $gt: 0 }, 'error': { $exists: false }, $or: [ { 'latest_sync': { $exists: false } }, { 'latest_sync': { $lt: new Date((new Date()) - (1000 * 60 * 10)) } } ] }, function (err, records) {
                if (err) {
