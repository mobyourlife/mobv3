...
module.exports = function (router) {
	...
	router.get('/login', enableCors, function(req, res) {
		if (req.isAuthenticated() && req.headers.referer) {
			...
			if (AdminPerms.isAdmin(req.user)) {
				res.send({
					auth: true,
					uid: req.user._id,
			...
			Domain.findOne({ '_id': parsed.hostname }, function(err, domain) {
				if (err) {
					throw err;
				}

				if (domain) {
					User.findOne({ _id: req.user._id, 'fanpages.id': domain.ref }, function (err, user) {
						if (err) {
							throw err;
						}
